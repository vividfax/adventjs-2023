...
let galaxykatebranchCount = 0
let galaxykatetreeCount = 0
...
class Day25 extends Day {
    ...
    prerun() {
        ...
        this.scene = new GalaxykateScene()
...
class GalaxykateScene {

    constructor() {
        this.x = 0
        this.trees = []
        this.flock = new GalaxykateFlock()

        let count = 7
        for (var i = 0; i < count; i++) {
            this.trees.push(new GalaxykateTree(new GalaxykateVector2D(10 + 120*i + 200*Math.random(),300)))
        }
        this.wind = new GalaxykateVector2D()
    }
...
class GalaxykateBranch {
    // Can contain other branches
    constructor({rootPoint,tree, parentBranch, branchIndex}) {

        this.lengthFalloff = .8
        this.radiusFalloff = .5
        this.idNumber = galaxykatebranchCount++
        ...
        this.points = []
        for (var i = 0; i < 10; i++) {
            let pt = Object.assign(new GalaxykateVector2D(), {
                radius:1,
                energy:1,
                index: i,
                id:"pt" + this.idNumber + "-" + i,
                velocity: new GalaxykateVector2D(0,0)
            })
            this.points.push(pt)
        }
        ...
        if (this.level < MAX_LEVEL) {
            ...
            for (var i = 0; i< count; i++) {
                ...
                if (pos) {
                    this.branches.push(new GalaxykateBranch({
                        parentBranch: this,
                        rootPoint: pos,
                        branchIndex: i,
    ...
    update({t, dt, p}) {
        ...
        this.tip = new GalaxykateVector2D(this.rootPoint)
        let branchEnergy = constrain(this.rootPoint.energy, 0, 1)
        let speed = 3*branchEnergy*(this.rootPoint.radius**.4)
        this.tip.v = GalaxykateVector2D.polar(speed, this.startAngle)
...
class GalaxykateTree {

    constructor(rootPoint) {
        this.idNumber = galaxykatetreeCount++
        this.size = .5 + Math.random()

        this.rootPoint = Object.assign(rootPoint, {
            energy: 10,
            velocity: new GalaxykateVector2D(0, -10),
            radius: 12*this.size
        })



        this.root = new GalaxykateBranch({rootPoint:rootPoint,tree:this})
...
class GalaxykateVector2D {
    static polar(r, theta) {
        return new GalaxykateVector2D(r*Math.cos(theta), r*Math.sin(theta))
    }

    static sub(pt0, pt1) {
        return new GalaxykateVector2D(pt0.x - pt1.x, pt0.y - pt1.y)
    }

    static edgePoint(...args) {
        return new GalaxykateVector2D().setToEdgePoint(...args)
    }
  ...
  static addMultiple(...args) {
    return new GalaxykateVector2D().addMultiple(...args)
}
...
clone() {
    return new GalaxykateVector2D(this.x, this.y);
}
...
getNormal() {
    ...
    return new GalaxykateVector2D(this.y/m, -this.x/m)
}
...
setToEdgePoint({pt0, pt1, v, pct=0, edgeOffset=0, normalOffset=0}) {
    ...
    if (!v)
        v = new GalaxykateVector2D( pt1.x - pt0.x, pt1.y - pt0.y)
...
addNormal(v0, v1, n) {
    let m = GalaxykateVector2D.distance(v0, v1)
...
drawArrow(p, {v, multiplyLength=1, normalOffset=0,  startOffset=0, endOffset=0, color}) {
        ...
        // Make points
    let start = GalaxykateVector2D.edgePoint({pt0:this, v, pct: 0, normalOffset, edgeOffset: startOffset})
    let end = GalaxykateVector2D.edgePoint({pt0:this, v, pct: multiplyLength, normalOffset, edgeOffset: endOffset})
...
class GalaxykateFlock {

    constructor() {
        this.boids = []

        this.center  = new GalaxykateVector2D(300, 200)

        for (var i = 0; i< 12; i++) {
            let v = GalaxykateVector2D.polar(Math.random()*200, Math.random()*100)
            v.add(this.center)
            v.boidAngle = Math.random()*100
            v.velocity = GalaxykateVector2D.polar(1900, v.boidAngle)
            v.force = new GalaxykateVector2D()
            this.boids.push(v)
            // console.log(v)
        }
